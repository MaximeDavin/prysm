...
func newTestTransport(t *testing.T) *Transport {
	priv, pub, err := crypto.GenerateSecp256k1Key()
	if err != nil {
		t.Fatal(err)
	}
...
func newTestTransportWithMuxers(t *testing.T, muxers []protocol.ID) *Transport {
...
func TestPeerIDMismatchOutboundFailsHandshake(t *testing.T) {
	...
	var mismatchErr ErrPeerIDMismatch
...
func TestEarlyDataAccepted(t *testing.T) {
	handshake := func(t *testing.T, client, server EarlyDataHandler) {
		t.Helper()
		initTransport, err := newTestTransport(t).WithSessionOptions(EarlyData(client, nil))
		require.NoError(t, err)
...
func TestEarlyfffDataAcceptedWithNoHandler(t *testing.T) {
	...
	initTransport, err := newTestTransport(t).WithSessionOptions(EarlyData(clientEDH, nil))
	require.NoError(t, err)
	respTransport := newTestTransport(t)
...
func TestHandshakeWithTransportEarlyData(t *testing.T) {
	...
	noiseHandshake := func(t *testing.T, initProtos, respProtos []protocol.ID, expectedProto protocol.ID) {
		initTransport := newTestTransportWithMuxers(t, initProtos)
		respTransport := newTestTransportWithMuxers(t, respProtos)
