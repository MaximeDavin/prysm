...
func Upgrade(ctx context.Context, t *TcpTransport, rawConn manet.Conn, pid peer.ID, direction transport.Direction) (transport.Conn, error) {
...
func upgradeSecurity(ctx context.Context, t *TcpTransport, rawConn manet.Conn, pid peer.ID, direction transport.Direction) (transport.SecureConn, error) {
	...
	var err error
	switch direction {
	case transport.DirInbound:
		proto, _, err = t.SecurityMuxer.Negotiate(rawConn)
	case transport.DirOutbound:
